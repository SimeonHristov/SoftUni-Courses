...
export class ServerService {
  ...
  constructor(private http: HttpClient) { } //inject HttpClient, use to make call to the backend to retrieve data and map to appState
      ...
      subscriber => {
        ...
        subscriber.next(
          status === Status.ALL ? { ...response, message: `Server filtered by ${status} status` } :
            {
              ...response,
              message: response.data.servers //TODO: turned tsconfig.json strict = false  <= FIX this
